...
L["CONFIG_HEADER_COACH"]      = "コーチ表示"
L["CONFIG_HEADER_ALERTS"]     = "アラート"
...
L["CONFIG_ICON_COUNT_DESC"]   = "メインスキルの横に表示する先読み予測アイコン数 (1-4)。"
L["CONFIG_ICON_SPACING"]      = "アイコン間隔"
L["CONFIG_ICON_SPACING_DESC"] = "アイコンバー内の隣接アイコン間のピクセル間隔。"
L["CONFIG_SCALE"]             = "スケール"
...
L["CONFIG_HIDE_CD_PREDICTIONS"]      = "クールダウン中の予測を隠す"
L["CONFIG_HIDE_CD_PREDICTIONS_DESC"] = "クールダウン中の予測スキルをグレー表示せず、そのまま非表示にします。"
L["CONFIG_SHOW_RANGE"]        = "射程インジケーターを表示"
L["CONFIG_SHOW_RANGE_DESC"]   = "ターゲットが射程外のとき、メインアイコンを赤く点滅させます。"
L["CONFIG_SHOW_PROC"]         = "プロック光彩を表示"
L["CONFIG_SHOW_PROC_DESC"]    = "Blizzard がプロックと判定したとき、メインアイコンを光らせます。"

-- コーチ表示
L["CONFIG_PHASE_INDICATOR"]      = "フェーズ表示"
L["CONFIG_PHASE_INDICATOR_DESC"] = "アイコンバーの上に戦闘フェーズのバッジを表示します。"
L["CONFIG_RESOURCE_BAR"]         = "リソースバーを表示"
L["CONFIG_RESOURCE_BAR_DESC"]    = "アイコンバーの下に主リソースバーを表示します。"
L["CONFIG_ACCURACY_METER"]       = "推奨一致率を表示"
L["CONFIG_ACCURACY_METER_DESC"]  = "詠唱が RotaAssist の推奨と一致した割合を表示します。DPS スコアではありません。"
L["CONFIG_PREPULL_PANEL"]        = "プル前チェックリストを表示"
L["CONFIG_PREPULL_PANEL_DESC"]   = "非戦闘時にプル前の準備チェックリストを表示します。"

-- アラート
L["CONFIG_DEFENSIVE_SOUND"]      = "防御スキルの音アラート"
L["CONFIG_DEFENSIVE_SOUND_DESC"] = "防御スキルが推奨されたときに警告音を鳴らします。"
L["CONFIG_INTERRUPT_SOUND"]      = "妨害の音アラート"
L["CONFIG_INTERRUPT_SOUND_DESC"] = "緊急度の高い妨害時に警告音を鳴らします。"
...
L["INDEPENDENT_USAGE"]       = "/ra independent on|off - 実験的な独立推奨を切り替え"
L["BUILD_USAGE"] = "/ra build - 現在のキャラクター構成を確認"
L["BUILD_READY"] = "完全"
L["BUILD_UNKNOWN"] = "構成が不完全、またはタレント変更が未確定のため、独立モードでビルドを確認できません。"
L["BUILD_STATUS"] = "専門 %d / 構成 %d / ヒーローサブツリー %s：選択 %d 項目、確認 %d ノード。"
L["BUILD_METADATA"] = "装備：%d 個（%s）。ローテーション・タレント呪文情報：%d 件。"
L["BUILD_STATS"] = "観測攻撃力 %s、ヘイスト %s%%、クリティカル %s%%、マスタリー効果 %s%%。"
L["BUILD_IMPORT"] = "タレントインポート：%s"
L["BUILD_LIMITS"] = "構成認識は DPS 検証ではありません。能力値と呪文情報は取得時点の値で、ダメージ係数や全タレント効果の校正は未完了です。"
L["INDEPENDENT_ENABLED"]     = "実験的な独立推奨を有効にしました。このポリシーは参照ベンチマークを上回っていません。状態が不明な場合は従来の推奨を使用します。"
L["INDEPENDENT_DISABLED"]    = "実験的な独立推奨を無効にしました。"
L["INDEPENDENT_BADGE"]       = "実験"
...
-- UI Toggles & Metrics
L["SHOW_ACCURACY_METER"]      = "推奨一致率を表示"
L["SHOW_PHASE_INDICATOR"]     = "フェーズ表示"
L["SHOW_RESOURCE_BAR"]        = "リソースバーを表示"
L["ACCURACY"]                 = "推奨一致率"
L["BLIZZARD_ACCURACY"]        = "Blizzard 推奨一致率"
L["SMART_ACCURACY"]           = "RotaAssist 推奨一致率"
...
-- スペック対応範囲（v1.1.0 はデーモンハンターのみ配信 —— D-014 参照）
------------------------------------------------------------------------
