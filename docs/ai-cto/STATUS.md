...
## 2026-09-22：1.1.1-rc.4 当前目标与范围观测

Astra 独立实现目标切换失效、浩劫近战距离计数、技能距离过滤，以及部分公开的
自身 Buff / 当前目标本人施加的 Debuff 到期时间。姓名板不再直接等同于范围内敌人。
实现与受限值、范围形状和 DPS 验收边界见 [交付说明](../RELEASE_1.1.1-rc.4.md)。
...
交付渠道为本地 ZIP，未发布公共 Release。修复主推荐与有序前瞻、未验证专精
门控、模块生命周期、资源条受限值处理、冷却面板空白及可回滚安装。
具体交付和验收边界见 [候选版说明](../RELEASE_1.1.1-rc.1.md) 与
[推荐质量标准](../QUALITY_BASELINE.md)。本机缺少正式服 `Wow.exe`，没有真机
或独立 DPS 对照结果；“理论最优/超越 Hekili”目标仍未验证。

下方为 Round 19 的历史状态，不能作为本轮现状或公开发布证明。
